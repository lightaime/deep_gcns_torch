...
class GraphConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_type='edge',
                 act_type='relu', norm_type=None, bias=True, aggr='max'):
        super(GraphConv, self).__init__()
        if conv_type == 'edge':
            self.gconv = EdgConv(in_channels, out_channels, act_type, norm_type, bias, aggr)
        elif conv_type == 'mr':
            self.gconv = MRConv(in_channels, out_channels, act_type, norm_type, bias, aggr)
...
class DynConv(GraphConv):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=9, dilation=1, conv_type='edge', act_type='relu',
                 norm_type=None, bias=True, aggr='max', stochastic=False, epsilon=1.0, knn_type='matrix'):
        super(DynConv, self).__init__(in_channels, out_channels, conv_type, act_type, norm_type, bias, aggr)
        self.k = kernel_size
        self.d = dilation
...
class ResDynBlock(nn.Module):
    ...
    def __init__(self, channels,  kernel_size=9, dilation=1, conv_type='edge', act_type='relu', norm_type=None,
                 bias=True, aggr='max', stochastic=False, epsilon=1.0, knn_type='matrix'):
        super(ResDynBlock, self).__init__()
        self.body = DynConv(channels, channels, kernel_size, dilation, conv_type,
                            act_type, norm_type, bias, aggr, stochastic, epsilon, knn_type)

    # input: (x0, x1, x2, ..., xi);  (xi-1, xi), output is (xi, xi+1)
    def forward(self, x, batch):
        res = self.body(x[-1], batch)
        res = res + x[-1]
        return torch.cat((x, res.unsqueeze(0)), 0), batch
...
class DenseDynBlock(nn.Module):
    ...
    def forward(self, x, batch):
        res = self.body(torch.cat((x[-2], x[-1]), 1), batch)
        return torch.cat((x, res.unsqueeze(0)), 0), batch
